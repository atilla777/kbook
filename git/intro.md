## Git - общие сведения
В папке проекта приложения, находящегося под контролем git (в которой инициализирован git репозитарий) содержатся
* непосредственно папка репазитория - .git
* рабочий каталог.
...
Область подготовленных файлов (staging area) представляет собой файл .git/index. В этом файле содержится информация о том, какие изменения (файлы) войдут комит (прондексированы).
#### Типы объектов git
В репозитарии git (папка .git) существуют следующие типы объектов
...
```
В этом случае оба изменения войдут файл (они будут соответсвующим образом помечены), и  текстовом редакторе можно будет выбрать нужное изменение.
...
Так как при rebase создается новая история комитов, воспроизводящих отличия текущей ветки от новой базы, то при каждом таком новом комите возможны ситуаци конфликтов, после разрешения которых нужно продолжить reabase командой:
```bash
git rebase --continue
```
#### Удаленные (remote) репазитории
Для создания общего репазитория используется опция --bare (пустой). Имя такого репазитория по соглашению должно заканчиваться на .git:
```bash
git init --bare имя.git
```
Особенности общего (bare) репазитория
* репазиторий помещается в корне папке, а не в .git
* рабочая директория в таком репозитории отсутствует

Просмотр имен удаленных репазиториев
```bash
git remote
```
Добавление удаленного репазитория (будет изменен файл .git/config)

```bash
git remote add имя путь
```
Клонирование удаленного репазитория
```bash
git clone путь
```
Отправка изменений в удаленный репазторий
* если ветка на удаленном репазитории не существует
```bash
git push репазиторий ветка
```
* если ветка существует
```bash
git push
```
Скачивание изменений из удаленного репазитория
```bash
git pull репазиторий ветка
```
Для создания новой ветки на общем репазитории ветка сначала создается в локальном, а затем выполняется
```bash
git push репазиторий новая ветка
```
Для удаления ветки на удаленном репазитории
```bash
git push репазиторий :ветка
```
Для того что бы находясь в какой либо локальной ветке, не указывать <репазиторий> <ветка> каждый раз при выполнении команд push, pull надо настроить отслеживание (set-upstream)
находясь в локальной ветке, связываемой с удаленной веткой –
```bash
git branch -u <удаленный репазиторий>/<удаленная ветка>
```
Указать git отправлять на удаленный репазиторий только текущую ветку, а не все отслеживаемые можно следующей настройкой
```bash
