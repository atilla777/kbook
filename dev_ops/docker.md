...
Надо не забывать очищать кэш пакетного менеджера (скченные для установки пакеты и т.п.) в каждом слое, то есть после каждой команды типа RUN apt-get install
```
RUN apt-get install tmux 
RUN apt-get install sudo
```
#### Исопльзотвание кэша образа
Команды добавляющие в образ часто изменяемыое содержимое (исходный код) помещается ниже пенееч часто изменяющих содержимое команд (установка пакетов).
### Docker compose
#### Установка
```bash
