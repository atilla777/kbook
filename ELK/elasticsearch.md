...
* master (master-eligible или выбираемый мастером) - узел, который участвует в выборе основного (master) узла кластера, который, в свою очередь, осуществляет управление кластером (учет членов кластера, управление индексами, размещение шард по узлам).
* data (узел данных) - хранение документов (шард) кластера и выполнение CRUD операций.
* coordinating (координирующий) - прием клиентских запросов, отправка заппросов к нужным узлам с данными, объединение и возврат клиенту результата. Эта роль неявно есть у всех узлов, что бы ее отключить надо запретить клиентские подключения к узлу.
